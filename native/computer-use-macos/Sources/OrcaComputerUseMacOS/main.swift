...
final class Provider {
    ...
    private func currentSnapshot(params: [String: JSONValue]) throws -> Snapshot {
        ...
        // One-shot CLI/stdio clients have no in-process get-app-state to compare
        // against; requested indexes resolve directly against the fresh tree
        // (element_not_found still fires for out-of-range indexes).
        if cached != nil {
...
func permissionStatusSnapshotSettled() -> PermissionStatusSnapshot {
...
// Standalone mode (--allow-standalone): line-delimited JSON-RPC on stdin/stdout.
...
private func runStandaloneStdio() {
    ...
    while let line = readLine(from: STDIN_FILENO) {
        ...
        guard !trimmed.isEmpty,
            ...
            writeJSON(["id": NSNull(), "ok": false, "error": ["code": "invalid_argument", "message": "invalid request line"]], to: nil)
            continue
        }
        if request.method == "terminate" {
            writeJSON(["id": request.id, "ok": true, "result": ["ok": true]], to: nil)
            exit(0)
        }
        let response = handleRequest(provider: provider, lock: lock, request: request, expectedToken: nil, authorizedPeer: true)
        writeJSON(response, to: nil)
    }
    exit(0)
}
...
} else if arguments.first == "--allow-standalone" {
    runStandaloneStdio()
} else if StandaloneCLI.isCommand(arguments.first) {
    StandaloneCLI.run(arguments)
} else {
    runStdio()
}
